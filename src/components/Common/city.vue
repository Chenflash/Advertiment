       ...
       <div v-cloak class="listContent" >
          ...
          <!--热门城市的选择-->
           <div class="hotCity" id="userHot" v-show="isSearch" v-if="useList.length>0" >
             <h3>常用</h3>
             <ul class="hotClass">
               <li v-for="(list,i ) of useList" :key="i" v-if="i<9" @click="getList(list)">{{list.length>=5?list.slice(0,4)+'...':list}}</li>
            </ul>   
          </div>  
          ...
          <div v-for="item in showCity" class="letter-item" >
            ...
            <div v-for="i in item.citylist" @click="getList(i)">{{i}} </div>
          </div>
...
export default {
  ...
  mounted(){
     ...
     if(window.localStorage.getItem('cityli')==null){
        this.useList=[];
     }else{
       this.useList=JSON.parse(window.localStorage.getItem('cityli')); 
    ...
    if(countPage<2){
       var domain = process.env.NODE_ENV == 'development' ?'http://' + window.location.host: 'http://tongcheng.intexh.com/wap/dist' ;
        axios({
           ...
           transformRequest: [function (data) {
            ...
            if(response!=undefined){
              if(response.status==200){
                let cityList=response.data.provinces,i=0,j=0; //省份
                for(let i in cityList){
                  cityLists=cityList[i].citys;  //获取城市数组
                  for(let j in cityLists){
                      cityNames.push(cityLists[j].citysName);   //获取城市名称
                  }
                }
          ...
          if(isiOS){
            document.getElementById("navHeader").classList.add("fixeds");
          }
    }
  ...
  methods:{
     buildLetter(){ //构建字母���,26个字母表排序
       ...
       this.buildLetter()
        ...
        for (let i = 0; i < cityNamesFilter.length; i++) {
          ...
          if (indexs >= 0 && indexs < 26) {
            letter[indexs].citylist.push(cityNamesFilter[i]);
      ...
      naver(id){ //点击右边字母滚动
       ...
       cityFilter:function(city){  //城市的搜索筛选
        ...
        showCity = showCity.filter(function (value) {
          ...
          value.citylist=Array.from(new Set(value.citylist))
      ...
      getLocation(val){   //获取热门地址
         if(val!="全国"){
          val=val+"市"
           this.$router.push({name:'merchantClassPage',query:{keys:this.$route.query.keys,dingweiCity:this.dingweiCity,falg:1},params:{gc_id:this.gc_id,getCity:val}});   
          this.useList.unshift(val);
          var listData=[];
          listData=Array.from(new Set(this.useList));
         this.userList=listData;
           if(listData.length>9){
             this.userList=listData.slice(0,9)
          }
          localStorage.setItem('cityli',JSON.stringify(this.userList));
      ...
      getList(i){ //获取城市的选择
        this.$router.push({name:'merchantClassPage',query:{keys:this.$route.query.keys,dingweiCity:this.dingweiCity,falg:1},params:{gc_id:this.gc_id,getCity:i}});
      ...
      getDingwei(e){  //定位
         this.$router.push({name:'merchantClassPage',query:{keys:this.$route.query.keys,dingweiCity:this.dingweiCity,falg:1},params:{gc_id:this.gc_id,getCity:this.dingweiCity}});
