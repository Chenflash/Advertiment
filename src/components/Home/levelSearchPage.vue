<template>
  <div class="levelSearchPage">
    <!--搜索-->
    <!--城市列表-->
   <header class="navHeader">
     <span class="leftIcon" @click="toback()"><img src="../../assets/left.png"/></span> 
  ...
  <div v-show="!isloading">
   ...
   <div v-if="!showValue" class="searchValue">
      <div v-if="levelCity.length!=0"><h3 class="searchTitle" >搜索历史</h3><img src="../../assets/delete.png" class="icons" @click="clearSearch"/></div>
    
      <ul class="searchs" v-if="levelCity.length!=0">
        <li v-for="(item,i) in levelCity" :key="i" @click="fetchList(item)">{{item}} </li>
     </ul>
   <!--暂无数据-->
   ...
   <div v-else >
     <ul class="searchList"  v-if="results.length>0">
                    ...
                    <p class="avatarList"><img v-lazy="item.store_avatar==null?require('../../assets/mine.png'):item.store_avatar" class="infoImage"  /></p>
...
export default {
  ...
  mounted() {
    //获取历史记录
    if(JSON.parse(localStorage.getItem('levelCity'))==null){
      this.levelCity=[];
    }else{
      this.levelCity=JSON.parse(localStorage.getItem('levelCity'));
    }
     var that=this;
  ...
  methods:{
    ...
    fetchSearch(){  //搜索列表
      if(this.keySearch!=""){
        ...
        this.levelCity.push(this.keySearch);
          var  listData=[],isFlag; //数组去重
          for(var i=0;i<this.levelCity.length;i++){
            isFlag=false;
              if(this.levelCity[i]!=''){
                for(var j=0;j<listData.length;j++){
                  if(this.levelCity[i]==listData[j]){
                  isFlag=true;
                  break;
                  } 
              }
              if(!isFlag){
                listData.push(this.levelCity[i]);
              }
            }
          }
          this.levelCity=listData;
          localStorage.setItem('levelCity',JSON.stringify(listData));
   ...
   clearSearch(){  //清空搜索记录
        MessageBox.confirm('','确定删除全部历史搜索?').then(action => {
          if (action == 'confirm') {
            window.localStorage.removeItem('levelCity');
            this.levelCity=[];
            this.showSearch=true;
    ...
    toback(){
     if(this.$route.query.isSelected==1){
           this.$router.push({name:'merchantLevelPage',query:{keys:this.$route.query.keys,dingweiCity:this.$route.query.dingweiCity,gc_id:this.$route.query.gc_id,getCity:this.$route.query.getCity,
           isSelected:1}});
         }else{
           this.$router.push({name:'merchantLevelPage',query:{keys:this.$route.query.keys,dingweiCity:this.$route.query.dingweiCity,gc_id:this.$route.query.gc_id,getCity:this.$route.query.getCity}});  
         }
