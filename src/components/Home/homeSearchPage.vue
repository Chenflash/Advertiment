<template>
  ...
  <div v-show="!isloading">
   ...
   <div v-if="!showValue" class="searchValue">
      <div v-if="homeCity.length!=0"><h3 class="searchTitle" >搜索历史</h3><img src="../../assets/delete.png" class="icons" @click="clearSearch"/></div>
    
      <ul class="searchs" v-if="homeCity.length!=0">
        <li v-for="(item,i) in homeCity" :key="i" @click="fetchList(item)">{{item}} </li>
     </ul>
   <!--暂无数据-->
...
export default {
  ...
  mounted() {
    //获取历史记录
    if(JSON.parse(localStorage.getItem('homeCity'))==null){
      this.homeCity=[];
    }else{
      this.homeCity=JSON.parse(localStorage.getItem('homeCity'));
    }
     var that=this;
  ...
  methods:{
    ...
    fetchSearch(){  //搜索列表
      if(this.keySearch!=""){
        ...
        this.homeCity.push(this.keySearch);
          var  listData=[],isFlag; //数组去重
          for(var i=0;i<this.homeCity.length;i++){
            isFlag=false;
              if(this.homeCity[i]!=''){
                for(var j=0;j<listData.length;j++){
                  if(this.homeCity[i]==listData[j]){
                  isFlag=true;
                  break;
                  } 
              }
              if(!isFlag){
                listData.push(this.homeCity[i]);
              }
            }
          }
          this.homeCity=listData;
          localStorage.setItem('homeCity',JSON.stringify(listData));
